...
% 		\vspace*{-2.5ex}
	...
	\note[item]{Thank the panel for attending.}
\end{frame}
%%%%%%%%%%%%% License %%%%%%%%%%%%%%
...
\begin{frame}<1-4>{Background \& Motivation}
    ...
    \begin{enumerate}
        ...
        \onslide<4-> \item<alert@+-> Variables are tightly coupled.
    \end{enumerate}
    
    \note<1->[item]{For that reason, an active area of research since the 70's has been to predict epileptic seizures using EEGs.}
    \note<1->[item]{A common paradigm which has been extensively studied, is supervised machine learning from labeled EEG segments.}
    \note<1->[item]{But, this paradigm suffers from two problems.}
    \note<2->[item]{First, labels are expensive to produce.}
    \note<3->[item]{second, EEG signal dynamics are more complex than the simplistic classes that experts have so far defined. Studies show that the labels are inexact, in the sense that disagreements between expert labelers are common in practice.}
    \note<4->[item]{Third, the annotations are tightly coupled to the EEG.}
    \note<4->[item]{In other words: untaped seizures. The annotation, which could be collected in a handwritten seizure diary, are useless to the supervised learning paradigm. These types of models can't learn from annotations without EEGs.}
    \note<4->[item]{Thus, partial or incomplete data points are completely ignored.}
\end{frame}
...
\note[item]{(1) Saab, Khaled, et al. "Weak supervision as an efficient approach for automated seizure detection in electroencephalography." NPJ digital medicine 3.1 (2020): 1-12.}
\note[item]{(2) Deiss, Olivier. Efficient labeling technique and interpretable deep neural network for the classification of seizures using continuous electroencephalograms. Diss. Georgia Institute of Technology, 2018.}
\note[item]{(3-prework) Yang, Yikai, et al. "Continental generalization of a human-in-the-loop AI system for clinical seizure recognition." Expert Systems with Applications 207 (2022): 118083.}
\note[item]{(3) Yang, Yikai, et al. "Weak self-supervised learning for seizure forecasting: a feasibility study." Royal Society Open Science 9.8 (2022): 220374.}
% 	\note[item]{previous approaches to merging planning in stochastic environments and probabilistic inference did not achieve optimal policies, and required fixing the episode length $T$ ahead of time.}
...
\begin{frame}{Data Collection}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.7\textwidth]{figs/MayoClinic_CanineEpilepsy_print.pdf}
        \caption{Canine Epilepsy Dataset. From: Coles et al. Feasibility study of a caregiver seizure alert system in canine epilepsy. Epilepsy Res. 2013 Oct; 106(3):456-60 Epub 2013. Used with permission of Mayo Foundation for Medical Education and Research, all rights reserved.}
        \label{fig:data}
    
        \note[item]{In a seminal work by a mixed team of engineers and veterinarians, a seizure advisory system was evaluated in canines with naturally occuring epilepsy.}
        ...
        \note[item]{475 days of data were made available online.}
    \end{figure}
\end{frame}

\begin{frame}{Simulating a prospective study}

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figs/empirical_evaluation_data_selection.pdf}
    \label{fig:my_label}
\end{figure}
\note[item]{This separation of concerns ensures simulating a real-time setting, in which the 'offline' calibration phase is followed by an 'online' seizure detection mode.}
...
\begin{frame}{Learned embeddings}
    \begin{figure}
        \centering
        \includegraphics[width=0.9\textwidth]{figs/embeddings_double.pdf}
...
\begin{frame}{Apx. 1 - Gaussian processes}
    ...
    \note[item]{$d$ is the distance between $x$ and $x'$ scaled by the \emph{lengthscale} parameter $\Phi$}
