...
\only<1>{
    ...
    \label{fig:my_label}
    \caption{Simulating a Prospective Study - Unsupervised Mode}
    \end{figure}
}
...
\only<2>{
    ...
    \label{fig:my_label}
    \caption{Simulating a Prospective Study - Weakly Supervised Mode}
    \end{figure}
}
...
\begin{frame}{Methods}
    ...
    \end{figure}
        ...
        \note[item]{Finally, a seizure probability is produced.}
\end{frame}
...
\begin{columns}[T]
    ...
    \begin{figure}
            ...
            \caption{Estimated $\hat{pdf_E}(h)$ using Gaussian mixture model}
            \label{fig:my_label}
        \end{figure}
    ...
    \note[item]{The next step is to estimate the density function }
    \note[item]{For computational speed ups, the GMM with 4 components was fit to a 5 dimensional reduced version of the original 8 dimensions.}
\end{frame}
...
\begin{frame}{Inference Procedure}
    ...
    \begin{block}{Computing Marginal Likelihood}
    \begin{align*}
    \label{eq:c3bsle:evidence}
    \prob(e^{new}) &= \frac{\text{Vol}(L_{e^{new}}\text{-HDR})}{\text{Vol}(\Omega_E)}
    \end{align*}
    \end{block}

    \begin{block}{Estimating w/ Monte Carlo}
        $$\prob(e^{new}) \gets \frac{\left|\{ e_i \mid \hat{p}(e^{new}) \le \hat{p}(e_i) \} \right|}{|\{ e_i\} |}$$
    \end{block}
    ...
    \end{columns}
    \note[item]{The probability of observing a sample $e^{new}$, is equal to the purple region divided by the entire region.}
...
\begin{frame}<1-2>{Priors}
    \vspace{-2\baselineskip}
    \begin{columns}
    \begin{column}{0.55\textwidth}
        \onslide<1->{
        \begin{block}{Non-informative prior}
            % $$\prob(\cancel{S_t}) = \lambda_0 \cdot T$$
            $$\prob(\cancel{S_t}) = C$$
        \end{block}
        }
        \onslide<1->{
        \begin{block}{Informative prior}
            \begin{align*}
            ...
            \begin{equation*}
            \prob(\cancel{S_t}) = \frac{1}{K}\sum_{i=0}^{23} f(t \mid i, k) 
            \end{equation*}
    ...
    % \end{block}
    \note<1->[item]{The uniform prior is a noninformative prior. Technically, it gives equal weight to each time point. My knowledge does not increase or degrade at any point.}
...
\begin{frame}<1,2>{Evaluating BSLE on validation data}
    ...
    \note<1->[item]{The lower region of $\alpha$ (i.e. most of $\Omega_E$ is in $\cancel{S}$, the weakly supervised model is a better predictor. The advantage disappears once we define the normal model $\cancel{S}$ to include ~96\% or less of the highest density region samples.}
\end{frame}
